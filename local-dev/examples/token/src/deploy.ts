...
async function main() {
  ...
  const contractModulePath = path.join(contractPath, "token", "contract", "index.cjs");

  if (!fs.existsSync(contractModulePath)) {
    console.error("Contract not found! Make sure token contract is compiled.");
  ...
  const zkConfigPath = path.join(contractPath, "token");
  const providers = {
